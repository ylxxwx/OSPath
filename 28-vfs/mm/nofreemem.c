...
u32 kmalloc_nofree(u32 sz) {
    ...
    //    return 0;
    //}
    // 0xA0000 is kernel stack for Process 0
    // init place stack on A0000 as well, but it move to first process so.
    // it should be OK to reuse this memory.
    if (free_mem_addr + sz >= 0xA0000) {
        kprintf("WARNING!!!! nofree memory reach 0xA0000\n");
        return 0;
    }
